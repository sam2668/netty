...
public class DatagramDnsResolverTest {
    ...
    @Test
    public void testLookupAll4() throws Exception {
        ...
        List<Inet4Address> addresses = dns.lookup4All("netty.io").get();
        Assert.assertEquals(1, addresses.size());
        Assert.assertEquals(ip, addresses.get(0).getHostAddress());
    }

    @Test
    public void testLookupAll6() throws Exception {
        AdvancedDnsResolver dns = prepare(new TestDnsServer(new RecordStore() {
            @Override
            public Set<ResourceRecord> getRecords(QuestionRecord questionRecord) {
                Set<ResourceRecord> set = new HashSet<ResourceRecord>();
        ...
        List<Inet6Address> addresses = dns.lookup6All("netty.io").get();
        Assert.assertEquals(1, addresses.size());
    ...
    public void testLookup4() throws Exception {
        ...
        Inet4Address address = dns.lookup4("netty.io").get();
    ...
    public void testLookup6() throws Exception {
        ...
        Inet6Address address = dns.lookup6("netty.io").get();
    ...
    public void testLookup() throws Exception {
        final String ip = "10.0.0.1";
        DnsResolver dns = prepare(new TestDnsServer(new RecordStore() {
            @Override
            public Set<ResourceRecord> getRecords(QuestionRecord questionRecord) {
                Set<ResourceRecord> set = new HashSet<ResourceRecord>();
    ...
    public void testLookupNonExisting() throws Exception {
        ...
        try {
            ...
            DnsResolverException ex = (DnsResolverException) e.getCause();
            Assert.assertTrue(ex.getCause() instanceof UnknownHostException);
        }
    }

    @Test
    public void testLookupTimeout() throws Exception {
        EventLoopGroup group = new NioEventLoopGroup(1);
        DatagramDnsResolverFactory factory = new DatagramDnsResolverFactory(NioDatagramChannel.class,
                NameServers.one(new InetSocketAddress("127.0.0.1", 5353)), 50, TimeUnit.MILLISECONDS);
        AdvancedDnsResolver resolver = (AdvancedDnsResolver) factory.resolver(group.next()).get();

        try {
            resolver.lookup("netty.io").get();
            Assert.fail();
        } catch (ExecutionException e) {
            e.getCause().printStackTrace();
            Assert.assertTrue(e.getCause() instanceof DnsResolverException);
        }
    }
    ...
    private AdvancedDnsResolver prepare(TestDnsServer server) throws Exception {
        ...
        DatagramDnsResolverFactory factory = new DatagramDnsResolverFactory(
                NioDatagramChannel.class, NameServers.one(addr));
        return (AdvancedDnsResolver) factory.resolver(group.next()).get();
    }
